...
#   4. Accepts all INPUT from LAN (ping, web UI :3000, QTS :8080, SSH, DNS, DHCP)
#
...
# -----------------------------------------------------------------------------
# 6. Allow LAN devices to reach the NAS itself
#    Without this, iptables silently drops ping, web UI, SSH etc. destined
#    for 192.168.0.150 when QTS has a non-ACCEPT default INPUT policy.
#    The LAN is a trusted internal segment so accepting all INPUT from eth1 is safe.
# -----------------------------------------------------------------------------
iptables -C INPUT -i "$LAN_IF" -j ACCEPT 2>/dev/null \
    || iptables -A INPUT -i "$LAN_IF" -j ACCEPT
log "LAN INPUT: all traffic from $LAN_IF ($LAN_IP) accepted"

# Also allow loopback (needed by Docker and local services)
iptables -C INPUT -i lo -j ACCEPT 2>/dev/null \
    || iptables -A INPUT -i lo -j ACCEPT
log "Loopback INPUT accepted"
